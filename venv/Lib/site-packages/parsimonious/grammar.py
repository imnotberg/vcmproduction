...
class RuleVisitor(NodeVisitor):
    ...
    def visit_parenthesized(self, parenthesized, xxx_todo_changeme):
        ...
        (left_paren, _1,
                                                  expression,
                                                  right_paren, _2) = xxx_todo_changeme
        return expression

    def visit_quantifier(self, quantifier, xxx_todo_changeme1):
        """Turn a quantifier into just its symbol-matching node."""
        (symbol, _) = xxx_todo_changeme1
        return symbol

    def visit_quantified(self, quantified, xxx_todo_changeme2):
        (atom, quantifier) = xxx_todo_changeme2
    ...
    def visit_not_term(self, not_term, xxx_todo_changeme4):
        (exclamation, term, _) = xxx_todo_changeme4
        return Not(term)

    def visit_rule(self, rule, xxx_todo_changeme5):
        """Assign a name to the Expression and return it."""
        (label, equals, expression) = xxx_todo_changeme5
    ...
    def visit_or_term(self, or_term, xxx_todo_changeme8):
        """Return just the term from an ``or_term``.

        We already know it's going to be ored, from the containing ``ored``.

        """
        (slash, _, term) = xxx_todo_changeme8
        return term

    def visit_label(self, label, xxx_todo_changeme9):
        """Turn a label into a unicode string."""
        (name, _) = xxx_todo_changeme9
        return name.text

    def visit_reference(self, reference, xxx_todo_changeme10):
        """Stick a :class:`LazyReference` in the tree as a placeholder.

        We resolve them all later.

        """
        (label, not_equals) = xxx_todo_changeme10
    ...
    def visit_literal(self, literal, xxx_todo_changeme12):
        """Pick just the literal out of a literal-and-junk combo."""
        (spaceless_literal, _) = xxx_todo_changeme12
    ...
    def visit_rules(self, node, xxx_todo_changeme13):
